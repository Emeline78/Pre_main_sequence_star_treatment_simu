...
"""
git add traitement_couple_automatization_adim.py
...
for path in all_dirs:
	...
	if snap_file.exists():
		...
		"""
		RS_snap = np.array(RS_snap)  * 2 * np.pi * r**2
		MS_snap = np.array(MS_snap)  * 2 * np.pi * r**2
		Visc_snap = np.array(Visc_snap) * 2 * np.pi * r**2
		MC_snap = np.array(MC_snap) * 2 * np.pi * r**2
...
df_final.to_parquet("transport_profiles_adim.parquet")
