...
"""
git add traitement_couple_automatization_sep_MS.py
...
for path in all_dirs:
	...
	if snap_file.exists():
		...
		MT_snap = np.array(MT_snap)
		Visc_snap = np.array(Visc_snap) 
		MC_snap = np.array(MC_snap) 
	
		save_snapshots(snap_dir,case_name,r,times,RS_snap,MS_snap,MT_snap,MC_snap,Visc_snap)
	...
	for i in range(len(dt)):
		...
		MT += 0.5*(MT_snap[i] + MT_snap[i+1])*dt[i]
...
df_final.to_parquet("transport_profiles_adim_sep.parquet")
