...
"""
git add scale_law_dim.py
git commit -m "modifications"
git push
"""

a = "transport_profiles_SI.parquet"
...
MS_rms = df.groupby("name").apply(lambda g: np.sqrt(np.mean(g["MS_SI"]**2))).to_numpy()
MS_int = df.groupby("name").apply(lambda g: np.trapz(g["MS_SI"], g["r_phys"])).to_numpy()
MS_min = df.groupby("name")["MS_SI"].apply(lambda x: x.iloc[x.argmin()]).to_numpy()
MS_max = df.groupby("name")["MS_SI"].apply(lambda x: x.iloc[x.argmax()]).to_numpy()

def MS_max_loc(g):
    r_mid = 0.5 * (g["r_phys"].min() + g["r_phys"].max())
    idx_mid = (g["r_phys"] - r_mid).abs().argmin()
    return g.iloc[idx_mid:]["MS_SI"].max()

MS_max_i = df.groupby("name").apply(MS_max_loc).to_numpy()

def MS_at_middle(g):
    r_mid = 0.5 * (g["r_phys"].min() + g["r"].max())
    idx = (g["r_phys"] - r_mid).abs().argmin()
    return g.iloc[idx]["MS_SI"]

MS_mid = df.groupby("name").apply(MS_at_middle).to_numpy()

neg_width = (df.groupby("name")["MS_SI"].apply(lambda x: np.sum(x < 0)).to_numpy())
Nmin = 10
...
Omega_phys = 2 * np.pi/ (10*24*3600) * Ek	# temps de ref, visqueux
liste = []
Rsun = 6.957e8
