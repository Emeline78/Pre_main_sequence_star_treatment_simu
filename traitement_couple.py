...
MS_max = np.max(MS_snaps,axis = 1)
snap = np.arange(1,len(MS_max)+1)

plt.figure()
plt.plot(snap,MS_mean, 'b', linewidth=2, label='MS mean')
plt.plot(snap,MS_median, 'r', linewidth=2, label='MS median')
plt.fill_between(snap,MS_min, MS_max, alpha=0.3)
plt.xlabel('Snapshot')
plt.ylabel('MS')
plt.legend()
plt.show()
